...
genestats_file = temp_dir.joinpath(f"tq_stats/{sys.argv[1]}_stats.parquet")
exonstats_file = temp_dir.joinpath(f"tq_exons/{sys.argv[1]}_exons.parquet")
nonexonstats_file = temp_dir.joinpath(f"tq_nonexons/{sys.argv[1]}_nonexons.parquet")
...
pogo_annotations = polars.scan_parquet(pogo_mapping_filtered_by_top_quadrant_contigs).with_columns(Chromosome = polars.lit("chr")+polars.col("Chromosome"))
